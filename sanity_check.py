...
def run_sanity_check(n_samples: int = 500, encoding: str = "motion"):
    ...
    print(f"    >0.5:   {sum(1 for m in modals if m > 0.5)} ({100*sum(1 for m in modals if m > 0.5)/len(modals):.1f}%)")
    print(f"    >0.7:   {sum(1 for m in modals if m > 0.7)} ({100*sum(1 for m in modals if m > 0.7)/len(modals):.1f}%)")
    ...
    buckets = [(0, 0.2), (0.2, 0.3), (0.3, 0.4), (0.4, 0.5), (0.5, 0.6), (0.6, 0.7), (0.7, 1.0)]
    ...
    if best_genotype is not None:
        ...
        evaluate_genotype(best_genotype, config, save_midi=True, midi_filename=midi_path)
